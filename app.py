...
if option == 'Deposit/Withdrawal':
    ...
    with col2:
        ...
        user_wallet_address = st.text_input("Enter the wallet address from where you want to transfer the ETH")
    ...
    with col2_2:
        ...
        user_wallet_address = st.text_input("Enter the wallet address to withdraw ETH to")
        ...
        if st.button("Withdraw"):
            tx_hash = contract.functions.withdrawEther().transact(
#                {'from': user_wallet_address, 'value':wei_withdraw_amount,'gas': 1000000}
                {'value':wei_withdraw_amount,'gas': 1000000}

                )
            # receipt = w3.eth.waitForTransactionReceipt(tx_hash)
            # st.write(receipt)
            st.markdown(f"{eth_withdraw_amount} ETH wihdrawn")        
